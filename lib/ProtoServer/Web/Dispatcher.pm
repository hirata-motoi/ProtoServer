package ProtoServer::Web::Dispatcher;
use strict;
use warnings;
use utf8;

use ProtoServer::Web::Router;
...
my $router = ProtoServer::Web::Router->new;
{
    for my $pl (dir(ProtoServer->base_dir . "/route")->children) {
...
{ # for access log
    my $fh = File::Stamped->new(pattern => ProtoServer->base_dir . "/log/access.log.%Y%m%d");
...
sub dispatch {
    ...
    my $web_root = $ENV{DEBUG_WEB_ROOT} || ref ProtoServer->context;
    my $req      = $c->request;
...
sub _filter {
    ...
    for my $filter ( @$filters ) {
        ...
        my $res_type = ref $res eq 'Amon2::Web::Response'  ? FILTER_RESULT_RESPONSE  :
                       ref $res eq 'ProtoServer::FormValidator' ? FILTER_RESULT_VALIDATOR : '';
        $results->{$res_type} = $res;
    }

    $results->{ FILTER_RESULT_VALIDATOR() } ||= ProtoServer::FormValidator->new($c->req);
    return $results;
}

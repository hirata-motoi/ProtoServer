package Test::ProtoServer::mysqld;
use strict;
use warnings;
use JSON;
use File::Spec;
...
sub create_database {
    ...
    Test::ProtoServer->diag("Created database: $dbname");
...
sub do_sql {
    ...
    Test::ProtoServer->diag("Loading SQL to $dbname");

    my $dbh = $self->dbh($dbname);
    chomp $sql;
    map { $dbh->do($_) } grep { $_ =~ /.+/ } split(/;/, $sql);

    Test::ProtoServer->diag("Loaded SQL to $dbname");
}

sub _create_q4m_functions {
    my ($self, $dbh, $dbname) = @_;

    Test::ProtoServer->diag("Install Q4M plugin: $dbname");
...
sub cleanup {
    ...
    for my $db (@$rows) {
        ...
        Test::ProtoServer->diag("Removed database: $db");
    }
}
